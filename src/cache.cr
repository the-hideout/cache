...
get "/api/cache" do |env|
  begin
    ...
    Log.error { "request to fetch an item from the cache failed: #{e.message} - #{e.backtrace}" }
    raise e
  end
end
...
post "/api/cache" do |env|
  begin
    ...
    Log.error { "request to save an item in the cache failed: #{e.message} - #{e.backtrace}" }
    raise e
  end
end
